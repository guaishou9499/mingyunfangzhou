...
func.is_time_rand_by_times = function(timestamp)
    ...
    local last_day_timestamp = this.get_timestamp_by_time_val(nil,nil,d + 0,4,5,0)
...
func.get_timestamp_by_time_val = function( year,month,day,hour,min,sec )
    ...
    -- 如果 day 大于 指定月的 天数,是否跨月
    local calc_m = month - this.get_time_mon()
    if calc_m > 0 then
        -- 读取月份大于当前月份【后N个月】
    elseif calc_m < 0 then
        -- 读取月份小于当前月份【前N个月】
    elseif calc_m == 0 then
        -- 本月
    end
    -- 获取指定月天数
    local m_day = tonumber(os.date("%d",os.time({ year = os.date("%Y"),month = os.date("%m") + calc_m + 1 ,day = 0 })))
    if day > m_day then
        -- 上月
        month = month - 1
        day   = m_day
