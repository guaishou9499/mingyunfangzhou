...
-------------------------------------------------------------------------------------------------------
-- 获取指定时间的时间戳是否在今日与下日指定时间段内[默认凌晨6点]
function utils.is_in_times(times,hour)
    hour             = hour or 6
    local day        = this.get_day()
    local cur_times  = this.get_timestamp_by_time_val(nil,nil,day + 0,0,0,0)
    local now_times  = this.get_timestamp_by_time_val(nil,nil,day + 0,hour,5,5)
    local next_times = this.get_timestamp_by_time_val(nil,nil,day + 1,hour,5,5)
    local check_time = times or os.time()
    return check_time >= now_times and check_time <= next_times or check_time >= cur_times and check_time <= now_times
end

-------------------------------------------------------------------------------------------------------
-- 获取指定时间的时间戳
-- @treturn      number                     时间戳
utils.get_timestamp_by_time_val = function( year,month,day,hour,min,sec )
    year      = year  or this.get_year()
    month     = month or this.get_month()
    day       = day   or this.get_day()
    hour      = hour  or this.get_hour()
    min       = min   or this.get_minute()
    sec       = sec   or this.get_second()
    -- 获取指定月天数
    local m_day = this.get_month_days(month)
    if day > m_day then
        -- 下月
        month = month + 1
        day   = day - m_day
...
-- @treturn      number                     月天数
------------------------------------------------------------------------------
function utils.get_month_days(month)
    local calc_m = month - this.get_month()
    local m_day  = tonumber(os.date("%d",os.time({ year = this.get_year(),month = month + calc_m + 1 ,day = 0 })))
    return m_day
end
